...
class TSnakeTail extends TSnakePart {
  constructor(aSpriteCanvas,aBoardCell) {
    ...
    this.prevBoardCell = new TBoardCell(aBoardCell.col, aBoardCell.row);
  }

  update(){
    
    this.prevBoardCell.col = this.boardCell.col;  //Store current position
    this.prevBoardCell.row = this.boardCell.row;
...
export class TSnake {
  ...
  #body = null;
  #tail = null;
  #growPending = 0; // added  to track segments to grow
  constructor(aSpriteCanvas, aBoardCell) {
  ...
  grow() {  //Makes the snake to grow every time a bait is eaten
